...
local unknownNoCompChk = CreateFrame("CheckButton", nil, rightCol, "UICheckButtonTemplate")
unknownNoCompChk:SetSize(22, 22); unknownNoCompChk:SetPoint("TOPLEFT", avoidPostedChk, "BOTTOMLEFT", 0, -2)
local unknownNoCompLbl = rightCol:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
unknownNoCompLbl:SetPoint("LEFT", unknownNoCompChk, "RIGHT", 2, 0)
unknownNoCompLbl:SetText("Count realms with no data as having no competition")
unknownNoCompChk:SetScript("OnEnter", function(self)
    GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
    GameTooltip:AddLine("Count realms with no data as having no competition")
    GameTooltip:AddLine("Realms showing |cff888888?|r in the listings column are ones TSM has no", 1, 1, 1, true)
    GameTooltip:AddLine("data for on this item - not realms known to be empty. By default the", 1, 1, 1, true)
    GameTooltip:AddLine("'No Competition' priority skips them, because no data is not the same", 1, 1, 1, true)
    GameTooltip:AddLine("as no sellers.", 1, 1, 1, true)
    GameTooltip:AddLine(" ")
    GameTooltip:AddLine("Turn this on and they count too, but only half as strongly - a realm", 1, 0.82, 0, true)
    GameTooltip:AddLine("checked and found empty still wins over one we simply don't know about.", 1, 0.82, 0, true)
    GameTooltip:AddLine(" ")
    GameTooltip:AddLine("Worth knowing: an item with no data on a realm sometimes means nobody", 0.9, 0.7, 0.4, true)
    GameTooltip:AddLine("trades it there - no competition, but no buyers either.", 0.9, 0.7, 0.4, true)
    GameTooltip:Show()
end)
unknownNoCompChk:SetScript("OnLeave", function() GameTooltip:Hide() end)
unknownNoCompChk:SetScript("OnClick", function(self)
    if ns.db then ns.db.settings.dfUnknownAsNoCompetition = self:GetChecked() end
    -- Changes what the ranking means, so re-rank rather than waiting for the
    -- next scan (FQ-253).
    if ReapplyPriority then ReapplyPriority() end
end)

local hideCommodityChk = CreateFrame("CheckButton", nil, rightCol, "UICheckButtonTemplate")
hideCommodityChk:SetSize(22, 22); hideCommodityChk:SetPoint("TOPLEFT", unknownNoCompChk, "BOTTOMLEFT", 0, -2)
...
function UI:RefreshDealFinderPage()
    ...
    elseif itemGroups and #itemGroups > 0 and currentIdx >= 1 then
        ...
        if ns.db then
            ...
            unknownNoCompChk:SetChecked(ns.db.settings.dfUnknownAsNoCompetition or false)
